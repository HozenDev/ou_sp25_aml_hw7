...
Andrew Justin 2025-04
  Added new sampling method to bias times closer to ground truth image
'''
import pandas as pd
import sys
import numpy as np
...
def load_single_file_wrapper(fname: str, image_size: [int] = (256, 256, 26)) -> [tf.Tensor]:
    ...
    # Possibly clip the image
    ins = tf.slice(ins, begin=[0,0,0], size=image_size)
    outs = tf.slice(outs, begin=[0,0], size=image_size[:2])
    

    # Add proper shape information to the TF Tensor
    ins = tf.ensure_shape(ins, image_size)
    outs = tf.ensure_shape(outs, image_size[:2])
...
def create_single_dataset(base_dir: str = '/home/fagg/datasets/radiant_earth/pa',
    ...
    if full_sat:
        ...
        # data = data.map(lambda x: tf.py_function(func=load_single_file_wrapper,
        #                                          inp=[x, [patch_size, patch_size, 26]], Tout=(tf.float32, tf.int8)),
        #                 num_parallel_calls=num_parallel_calls)
        data = data.map(lambda x: load_single_file_wrapper(x, [patch_size, patch_size, 26]),
                        num_parallel_calls=num_parallel_calls)
    ...
    if shuffle is not None:
        print("Shuffle", shuffle, type(shuffle))
...
def create_datasets(base_dir: str = '/home/fagg/datasets/radiant_earth/pa',
    ...
    ds_train = create_single_dataset(base_dir=base_dir,
                                     ...
                                     fold=fold,
                                     filt=train_filt,
                                     cache_path=None if cache_dir is None else '' if cache_dir == '' else '%s/train_f_%d_' % (
                                     cache_dir, fold),
                                     repeat=repeat_train,
    ...
    ds_valid = create_single_dataset(base_dir=base_dir,
                                     ...
                                     fold=fold,
                                     filt='*9',
                                     cache_path=None if cache_dir is None else '' if cache_dir == '' else '%s/validation_f_%d_' % (
                                     cache_dir, fold),
                                     repeat=False,
                                     shuffle=None,
                                     batch_size=batch_size,
    ...
    ds_test = create_single_dataset(base_dir=base_dir,
                                    ...
                                    fold=fold,
                                    filt='*',
                                    cache_path=None if cache_dir is None else '' if cache_dir == '' else '%s/test_f_%d_' % (
                                    cache_dir, fold),
                                    repeat=False,
                                    shuffle=None,
                                    batch_size=batch_size,
...
@tf.autograph.experimental.do_not_convert
def create_diffusion_example(I: tf.float32, L: tf.float32, patch_size: int, alpha: tf.Tensor, t: tf.Tensor) -> [
    tf.Tensor]:
...
@tf.autograph.experimental.do_not_convert
def create_diffusion_dataset(alpha:np.array,
                             base_dir:str='/home/fagg/datasets/radiant_earth/pa',
                             patch_size:int=256,
                             fold:int=0,
                             filt:str='*[012345678]',
                             cache_dir:str=None,
                             repeat:bool=False,
                             shuffle:int=None,
                             repeat_validation:bool=False,
                             shuffle_validation:int=None,
                             batch_size:int=8,
                             prefetch:int=2,
                             num_parallel_calls:int=4,
                             time_sampling_exponent:float=0.0):
    '''
    Use for HW 7
    ...
    :param prefetch: Number of batches to prefetch in parallel with training
    :param num_parallel_calls: Number of threads to use for the data loading process (-1 = use AutoTune)
    :param time_sampling_exponent: Time sampling bias exponent.  0 = uniform; 0.5 = heavily biased
    ...
    ##### TODO: NEW #####
    # Compute biased sampling distribution for time 
    time_array = np.linspace(0, nalpha-1, nalpha)
    time_tensor = tf.convert_to_tensor((time_array[::-1] + 0.5) / np.sum(time_array))[np.newaxis, ...] ** time_sampling_exponent
    # Log of the probabilities
    logits = tf.math.log(time_tensor)
    ...
    ds = ds.map(lambda I, L: create_diffusion_example(I[..., :3], L, patch_size, alpha_tf,
                                                      tf.random.categorical(logits, 1)[0]),
                num_parallel_calls=num_parallel_calls)
    ...
    ds_valid = ds_valid.map(lambda I, L: create_diffusion_example(I[..., :3], L, patch_size, alpha_tf,
                                                                  tf.random.categorical(logits, 1)[0]),
                num_parallel_calls=num_parallel_calls)
    ...
    ds_valid = ds_valid.map(
        lambda L, T, image, noise: ({'label_input': L, 'time_input': T, 'image_input': image}, noise))
